...
model_name2 = "llm-jp/llm-jp-3-980m-instruct3"
# device_map をサンプルの時点から実装してるのはとても偉いと思う（当たり前か）
qwen_model = AutoModelForCausalLM.from_pretrained(
    model_name1, torch_dtype="auto", device_map="auto"
)
...
with torch.no_grad():
    ...
    for i in range(max_length):
        if qwen_generating:
            ...
            qwen_prob_list.append(qwen_next_token_logits[qwen_next_token_id].item())
            ...
            qwen_input_ids = torch.cat(
                [qwen_input_ids, qwen_next_token_id.unsqueeze(0).unsqueeze(0)], dim=-1
            )
        ...
        if jp_generating:
            ...
            jp_prob_list.append(jp_next_token_logits[jp_next_token_id].item())
            jp_next_token = jp_tokenizer.decode(jp_next_token_id)
            jp_tokens.append(jp_next_token)
            if jp_next_token_id.item() == jp_eos_token_id:
                jp_generating = False
            jp_input_ids = torch.cat(
                [jp_input_ids, jp_next_token_id.unsqueeze(0).unsqueeze(0)], dim=-1
            )
...
print(qwen_prob_list)
print(" ".join(jp_tokens))
print(jp_prob_list)

minimum_len = min(len(qwen_prob_list), len(jp_prob_list))
for q_t, q_p, j_t, j_p in zip(qwen_tokens[:minimum_len], qwen_prob_list[:minimum_len], jp_tokens[:minimum_len], jp_prob_list[:minimum_len]):
    strs = [q_t, q_p, j_t, j_p]
